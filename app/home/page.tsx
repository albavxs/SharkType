...
interface QuickPracticePayload {
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const response = await fetch(`/api/practice/quick?languageId=${encodeURIComponent(selectedLanguageId)}`, {
          cache: 'no-store',
        })
        const payload = (await response.json()) as QuickPracticePayload

        if (!active) return
        if (!response.ok || !payload.selectedLanguage || !payload.snippets?.length) {
    ...
    <main className="flex-1 flex flex-col min-h-screen relative">
      ...
      <div className="relative z-10 flex-1 flex flex-col min-h-screen">
        ...
        <Toolbar language={language ?? initialLanguageMeta} difficulty={difficulty} seconds={displaySeconds}
          ...
          onDifficultyChange={handleDifficultyChange} onHelpClick={() => setShowHelp(true)}
